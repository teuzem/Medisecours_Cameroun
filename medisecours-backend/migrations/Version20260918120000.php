...
final class Version20260918120000 extends AbstractMigration
{
    ...
    public function up(Schema $schema): void
    {
        ...
        $this->addSql('ALTER TABLE "user" ADD COLUMN IF NOT EXISTS etablissement_nom VARCHAR(255) DEFAULT NULL');
        $this->addSql('ALTER TABLE "user" ADD COLUMN IF NOT EXISTS fonction VARCHAR(120) DEFAULT NULL');

        // ── centre_de_sante : traçabilité de la synchronisation ──────────
        $this->addSql("ALTER TABLE centre_de_sante ADD COLUMN IF NOT EXISTS source VARCHAR(20) DEFAULT 'manuel' NOT NULL");
        $this->addSql('ALTER TABLE centre_de_sante ADD COLUMN IF NOT EXISTS last_synced_at TIMESTAMP(0) WITHOUT TIME ZONE DEFAULT NULL');
        $this->addSql('CREATE INDEX IF NOT EXISTS IDX_CENTRE_GOOGLE_PLACE ON centre_de_sante (google_place_id)');
        $this->addSql('CREATE INDEX IF NOT EXISTS IDX_CENTRE_SOURCE ON centre_de_sante (source)');
    }

    public function down(Schema $schema): void
    {
        $this->addSql('DROP INDEX IDX_CENTRE_SOURCE');
        $this->addSql('DROP INDEX IF EXISTS IDX_CENTRE_GOOGLE_PLACE');
