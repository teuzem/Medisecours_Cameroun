...
export function hasMapsServerConfiguration() {
...
function getMapsConfig(): MapsConfig {
  ...
  if (!baseUrl || !apiKey) {
    throw new MapsServiceError(
      ...
      "Le proxy cartographique Manus n’est pas configuré. Définissez BUILT_IN_FORGE_API_URL et BUILT_IN_FORGE_API_KEY dans l’environnement serveur."
    );
  }
...
export const MAPS_QUOTA_COOLDOWN_MS = 5 * 60 * 1000;
let quotaCooldownUntil = 0;

export class MapsServiceError extends Error {
  constructor(public readonly kind: "configuration_missing" | "quota_exhausted" | "upstream_error", message: string) {
    super(message);
    this.name = "MapsServiceError";
  }
}

export function isMapsQuotaExhausted(status: number, detail: string) {
  return status === 412 && /usage exhausted|quota|resource exhausted/i.test(detail);
}

export function getMapsQuotaCooldownRemaining(now = Date.now()) {
  return Math.max(0, quotaCooldownUntil - now);
}
...
export async function makeRequest<T = unknown>(
  ...
  const cooldownRemaining = getMapsQuotaCooldownRemaining();
  if (cooldownRemaining > 0) {
    throw new MapsServiceError("quota_exhausted", `Le service cartographique est temporairement indisponible après épuisement du quota. Nouvelle tentative automatique dans environ ${Math.ceil(cooldownRemaining / 60_000)} minute(s).`);
  }
  ...
  if (!response.ok) {
    ...
    if (isMapsQuotaExhausted(response.status, errorText)) {
      quotaCooldownUntil = Date.now() + MAPS_QUOTA_COOLDOWN_MS;
      throw new MapsServiceError("quota_exhausted", "Le quota cartographique est temporairement épuisé. Les résultats Places seront de nouveau essayés automatiquement après une courte pause.");
    }
    throw new MapsServiceError("upstream_error", `Google Maps API request failed (${response.status} ${response.statusText}): ${errorText}`);
  }
